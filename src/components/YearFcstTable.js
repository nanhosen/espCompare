...
export default function YearFcstTable(props){
  const {station} = props 
  ...
  useEffect(()=>{
    // console.log('yearly table saw data type change, ', context.dataType)
    if(context.stationCompareDates && context.stationCompareDates[station] && context?.agnosticStationData[station]){
      console.log('station', station)
      console.log('context.stationCompareDates', context.stationCompareDates)
      const dataDisplayType = context.dataType === 'both' ? 'raw' :  context.dataType
      // console.log('yearly table saw data type change, in iff', dataDisplayType)
      const {date1, date2} = context.stationCompareDates[station][dataDisplayType]
      console.log('date1, date2', date1, date2)
      console.log('i am undefined', date1 !== undefined , date2 !== undefined)
      const definedDate1 = date1 ?? null;
      const definedDate2 = date2 ?? null;
      if(definedDate1 && definedDate2){
        ...
        // console.log('setting table data',dataByFcstYear)
        setTableData(dataByFcstYear)
      }else{
          console.log('not doing antying because missing a date',definedDate1,definedDate2 )
      }
  ...
  if(tableData){
    // console.log('table data for the rows', tableData)
    return(
...
function WaterYearRows(rowProps){
  ...
  //     const monthName = monthNameKey[fcstMonth]
  //     const yearName = String(fcstYear).slice(-2)
  //     const dataKey = `${monthName}${yearName}`
  //     console.log('dataKey', dataKey)
  //     console.log('fcstMonth', fcstMonth, 'year', fcstYear, 'data', fcstMonthVolume)
  //     if(!tableDataObjDataArray.hasOwnProperty(dataKey)){
  //       tableDataObjDataArray[dataKey]={
  //         data:[],
  //         simDates:[]
  //       }
  //     }
  //     tableDataObjDataArray[dataKey]['simDates'].push(issuanceDate)
  //     tableDataObjDataArray[dataKey]['data'].push(fcstMonthVolume)

  //     if(!tableDataObjDataObject.hasOwnProperty(dataKey)){
  //       tableDataObjDataObject[dataKey]=Object.create({})
  //       }
  //       tableDataObjDataObject[dataKey][issuanceDate]=fcstMonthVolume
  //   }
