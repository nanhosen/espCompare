    ...
    if(context.stationCardList &&context.stationCardList.length>0){
      return(
        ...
        context.stationCardList.map((currStation,i)=>{
          return(
            <>
              <TableGroup   key={i} station={currStation}/>
            </>
          )
        })
      )
...
function TableGroup(props){
  const {station} = props 
  // console.log('table groupo props', props)
  ...
  useEffect(()=>{
    // const functionFormatted = formatTableData(station,context.agnosticStationData[station], context.stationCompareDates[station] )
    // console.log('functionformatted', functionFormatted)
    const {date1, date2} = context.stationCompareDates[station]
    const dataDisplayType = context.dataType === 'both' ? 'adj' :  context.dataType
    // console.log('contezt', context)
    // console.log('dataDisplayType', dataDisplayType)
    ...
    // console.log(month1Info, month2Info)
    ...
    const month2Years = Object.keys(month2Info)
  ...
  },[context.agnosticStationData, context.stationCompareDates, context.dataType,context.dataType,props])
    ...
    return(
      <Grid item xs={6} md={6} lg={6}   component="div">
        ...
        <Typography align="left" variant='h6'>
          Chance of Exceeding River Levels for: {station} ({ context.dataType === 'both' ? 'adj' :  context.dataType})
        </Typography>
